...
class conv_ogg_mp3:
    ...
    def convertir(self):
        ...
        try:
            cambios = self.ogg_mp3(dir_in, dir_out, self.bitrate)
            self.label_result.config(text=cambios)
        except Exception as e:
            self.label_result.config(text=f"Error: {e}")
    
    def ogg_mp3(self, dir_in, dir_out=None, bitrate="192k"):
        ...
        resultado = f"🎵 Conversión completa: {contador} archivos convertidos en '{dir_out}'"
        return resultado
